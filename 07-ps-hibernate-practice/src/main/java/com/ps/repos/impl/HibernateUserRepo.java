...
public class HibernateUserRepo implements UserRepo {
    ...
    public List<User> findAllByUserName(String username, boolean exactMatch) {
        if (exactMatch) {
            ArrayList<User> list = (ArrayList<User>) session().createQuery("from User u where username = :username");
            return list;// TODO 36. Add Hibernate query to extract wll users with username = :username
        } else {
    ...
    public long countUsers() {
        int res = (int) session().createQuery("select count(u) from User u").uniqueResult();
        return res; // TODO 37. Add query to count all users
    }
